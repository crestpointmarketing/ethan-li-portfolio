...
import { aboutContentSchema } from '../../shared/siteContentSchema.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json({ about: data?.data ?? null });
  }
  ...
  if (req.method === 'PUT') {
    ...
    const { error } = await supabase
      .from('site_content')
      .upsert({ key: 'about', data: parsed.data }, { onConflict: 'key' });
    if (error) {
      console.error('PUT /api/admin/about', error);
      return res.status(500).json({ error: 'Failed to save content' });
    }
    return res.status(200).json({ about: parsed.data });
  }
